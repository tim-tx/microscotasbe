...
for c in controls.keys():
    ...
    for j in range(4):          # stitch number
        ...
        r = skio.imread(glob(path + controls[c] + '_03_3_%d_Texas Red_%03d.tif' % (j+1,1))[0])
        g = skio.imread(glob(path + controls[c] + '_03_2_%d_GFP_%03d.tif' % (j+1,1))[0])
        b = skio.imread(glob(path + controls[c] + '_03_4_%d_TagBFP_%03d.tif' % (j+1,1))[0])
        images = {'r':r,'g':g,'b':b}
